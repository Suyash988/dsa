...
class Solution {
    public void findCombinationSum(int ind, int target,int[] candidates,List<Integer> ds, List<List<Integer>> ans){
        // base condition
        if(ind == candidates.length){
            // when the target becomes zero
            if(target==0){
        ...
        if(candidates[ind]<=target){
          ...
          // to backtrack remove the last added element.
          ds.remove(ds.size()-1);
        }
